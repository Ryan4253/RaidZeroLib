...
CoordinateSystem::CoordinateSystem(const CoordinateAxis& iX, const CoordinateAxis& iY,
                                   const CoordinateRotation& iRotation)
    : x(iX), y(iY), rotation(iRotation) {
...
const CoordinateSystem& CoordinateSystem::Standard() {
    static const auto system =
        CoordinateSystem(CoordinateAxis::E(), CoordinateAxis::N(), CoordinateRotation::Counterclockwise());
    return system;
}

Rotation CoordinateSystem::convert(const Rotation& iRotation, const CoordinateSystem& from,
                                   const CoordinateSystem& to) {
    return iRotation * from.rotation.scale / from.rotation.scale;
}

Point CoordinateSystem::convert(const Point& iPoint, const CoordinateSystem& from, const CoordinateSystem& to) {
    // Transforms point to standard basis
    const Point transform =
        Point(iPoint.X() * from.x.c1 + iPoint.Y() * from.y.c1, iPoint.X() * from.x.c2 + iPoint.Y() * from.y.c2);
    ...
    return Point(transform.X() * xc1 + transform.Y() * yc1, transform.X() * xc2 + transform.Y() * yc2);
}

Pose CoordinateSystem::convert(const Pose& iPose, const CoordinateSystem& from, const CoordinateSystem& to) {
