...
QTime MockTimer::millis() const {
    return std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now() - epoch)
               .count() *
           millisecond;
}
...
std::unique_ptr<SettledUtil> createSettledUtilPtr(const double iatTargetError, const double iatTargetDerivative,
                                                  const QTime iatTargetTime) {
    return std::make_unique<SettledUtil>(std::make_unique<MockTimer>(), iatTargetError, iatTargetDerivative,
                                         iatTargetTime);
}

TimeUtil createTimeUtil() {
    return TimeUtil(Supplier<std::unique_ptr<AbstractTimer>>([]() { return std::make_unique<MockTimer>(); }),
...
TimeUtil createConstantTimeUtil(const QTime idt) {
    return TimeUtil(
        Supplier<std::unique_ptr<AbstractTimer>>([=]() { return std::make_unique<ConstantMockTimer>(idt); }),
        Supplier<std::unique_ptr<AbstractRate>>([]() { return std::make_unique<MockRate>(); }),
        Supplier<std::unique_ptr<SettledUtil>>([]() { return createSettledUtilPtr(); }));
}

TimeUtil createTimeUtil(const Supplier<std::unique_ptr<AbstractTimer>>& itimerSupplier) {
    return TimeUtil(itimerSupplier,
                    Supplier<std::unique_ptr<AbstractRate>>([]() { return std::make_unique<MockRate>(); }),
                    Supplier<std::unique_ptr<SettledUtil>>([]() { return createSettledUtilPtr(); }));
}
...
MockAsyncPosIntegratedController::MockAsyncPosIntegratedController()
    : AsyncPosIntegratedController(std::make_shared<MockMotor>(), AbstractMotor::gearset::green, 200,
                                   createTimeUtil()) {
}

MockAsyncPosIntegratedController::MockAsyncPosIntegratedController(const TimeUtil& itimeUtil)
    : AsyncPosIntegratedController(std::make_shared<MockMotor>(), AbstractMotor::gearset::green, 200, itimeUtil) {
}
...
MockAsyncVelIntegratedController::MockAsyncVelIntegratedController()
    : AsyncVelIntegratedController(std::make_shared<MockMotor>(), AbstractMotor::gearset::green, 200,
                                   createTimeUtil()) {
}
...
MockIterativeController::MockIterativeController() : IterativePosPIDController(0, 0, 0, 0, createTimeUtil()) {
}
...
void assertMotorsGearsetEquals(const AbstractMotor::gearset expected, const std::initializer_list<MockMotor>& motors) {
...
void assertAsyncControllerFollowsDisableLifecycle(AsyncController<double, double>& controller,
    ...
    EXPECT_EQ(domainValue, expectedOutput) << "Re-enabling the controller should move the motor to the previous target";
    ...
    EXPECT_EQ(domainValue, 1337) << "Re-enabling the controller after a reset should not move the motor";
}
